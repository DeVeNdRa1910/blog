...
def create_user(request: User, db: Session = Depends(get_db)):
    try:
        ...
        if isUserExist:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"Email ID already exist.")
        ...
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"Failed to create user.")
...
def update_partial_user_data(user_id, request: User, db: Session = Depends(get_db)):
    try:
        user = db.query(models.User).filter(models.User.id == user_id).first()
...
def update_complete_user_data(user_id, request: User, db: Session = Depends(get_db)):
    user = db.query(models.User).filter(models.User.id == user_id).first()
